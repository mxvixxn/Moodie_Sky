...
struct ContentView: View {
  ...
  @State var showDeleteScopeConfirm = false
  ...
  var tabs: some View {
    ...
    .alert("선택한 기록을 삭제할까요?", isPresented: $showDeleteScopeConfirm) {
      Button("삭제", role: .destructive) { performPendingDataDelete() }
      Button("취소", role: .cancel) {}
    } message: {
      Text(pendingDeleteScope == .today ? "오늘 남긴 기록을 모두 삭제해요." : "이번 달 기록을 모두 삭제해요.")
    }
    .fileImporter(
